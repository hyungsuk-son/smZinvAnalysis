...
//  SH::scanDQ2 (sh, "data15_13TeV.00270816.physics_Main.merge.AOD.f611_m1463");
  ...
  std::string myTagName = "xAOD.11172017.mc15c.sm.study.full.re";
  std::string containerName;
  std::vector< std::string > outputContainerNames; //for grid only

  std::ifstream inFile( "/cluster/home/h/s/hson02/atlas09Home/Work/vbfZinvAnalysis/smZInvAnalysis/util/mc15c_dataset.txt" );
  while(std::getline(inFile, containerName) ){
    if (containerName.size() > 1 && containerName.find("#") != 0 ){
      ...
      outputContainerNames.push_back( ("user.hson."+myTagName+datasetNum+tagName+"_hist/") );
      std::cout << "Printing output container name : " << "user.hson."+myTagName+datasetNum+tagName+"_hist/" << std::endl;
    }
  }
